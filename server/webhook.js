...
app.get('/health', (req, res) => {
    res.json({ status: 'ok', captured: callbacks.length });
});

if (require.main === module) {
    app.listen(PORT, () => {
        console.log(`⌬ Webhook receiver on port ${PORT}`);
        console.log(`📍 Capture URL: http://localhost:${PORT}/api/webhook/capture`);
        console.log(`🔍 Latest: http://localhost:${PORT}/api/webhook/latest`);
    });
}
